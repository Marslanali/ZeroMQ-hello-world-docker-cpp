...
void SubscriberThread1(zmq::context_t *ctx) {
    ...
    while (1) {
        ...
        std::cout << "SubscriberThread1: [" << recv_msgs[0] << "] "
...
void SubscriberThread2(zmq::context_t *ctx) {
    ...
    while (1) {
        ...
        std::cout << "SubscriberThread2: [" << recv_msgs[0] << "] "
                  << recv_msgs[1] << std::endl;
    }
}
...
void SubscriberThread3(zmq::context_t *ctx) {
    ...
    while (1) {
        ...
        std::cout << "SubscriberThread3: [" << recv_msgs[0] << "] "
                  << recv_msgs[1] << std::endl;
    }
}
...
/*
    ...
    auto thread = std::async(std::launch::async, PublisherThread, &ctx);

    // Give the publisher a chance to bind, since inproc requires it
    std::this_thread::sleep_for(std::chrono::milliseconds(10));

    auto thread1 = std::async(std::launch::async, SubscriberThread1, &ctx);
